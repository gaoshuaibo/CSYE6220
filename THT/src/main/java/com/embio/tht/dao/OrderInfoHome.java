 ...
 */
public class OrderInfoHome {

	private static final Log log = LogFactory.getLog(OrderInfoHome.class);
	...
	public void persist(OrderInfo transientInstance) {
	...
	public OrderInfo merge(OrderInfo detachedInstance) {
		...
		try {
			OrderInfo result = (OrderInfo) sessionFactory.getCurrentSession()
	...
	public OrderInfo findById(int id) {
		...
		try {
//			OrderInfo instance = (OrderInfo) sessionFactory.getCurrentSession()
//					.get("OrderInfo", id);
			Session session = sessionFactory.openSession();
			OrderInfo instance = (OrderInfo) session.get("com.embio.tht.beans.OrderInfo", id);
			if (instance == null) {
	...
	public List findByExample(OrderInfo instance) {
		...
		try {
			Session session = sessionFactory.openSession();
			List results = session.createCriteria("com.embio.tht.beans.OrderInfo").add(Example.create(instance))
					.list();
	...
	public OrderInfo findFirstByExample(OrderInfo instance) {
		log.debug("finding first UserInfo instance by example");
		try {
			List<OrderInfo> items = findByExample(instance);
